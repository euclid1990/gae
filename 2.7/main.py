...
def do_something_later(timestamp):
    ...
    if os.getenv("GAE_ENV", "").startswith("standard"):
        ...
        path = "/tmp"
        with open("/tmp/do_something.log".format(path=path), "a+") as f:
...
def about():
    try:
        ...
        logging.exception("An exception occurred: {ex}".format(ex=str(e)))
...
@app.route("/pushq")
def pushq():
    logging.info("Enqueue task to PUSH queue")
...
@app.route("/pullq")
def pullq():
    logging.info("Enqueue task to PULL queue")
    timestamp = get_unix_timestamp()
...
@app.route("/depullq")
def depullq():
    logging.info("Dequeue PUSH QUEUE task")
    HOUR = 3600
    TASKS = 1000
...
def defer():
    ...
    deferred.defer(do_something_later, timestamp, _countdown=5, _retry_options=retry)
